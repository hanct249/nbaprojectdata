...
champOdds2018 <- function(adjustedNet) {
  return(1/(1 + exp(-(-5.4108+(59.4572*adjustedNet)))))
}

predictionFinals <- glm(madeFinals ~ AdjustedNet, data = predicting, family = "binomial")
summary(predictionFinals)
finalsOdds2018 <- function(adjustedNet) {
  return(1/(1 + exp(-(-4.1572+(49.1897*adjustedNet)))))
}
predictionConfFinals <- glm(madeConfFinals ~ AdjustedNet, data = predicting, family = "binomial")
summary(predictionConfFinals)
confFinalsOdds2018 <- function(adjustedNet) {
  return(1/(1 + exp(-(-3.5510+(58.3616*adjustedNet)))))
}
predictionSecondRound <-glm(madeSecondRound ~ AdjustedNet, data = predicting, family = "binomial")
summary(predictionSecondRound)
secondRoundOdds2018 <- function(adjustedNet) {
  return(1/(1 + exp(-(-2.2030+(63.6691*adjustedNet)))))
}

predicting <- predicting %>%
  mutate(championOdds = ifelse(Season == "2017-18", champOdds2018(AdjustedNet), 0))
predicting <- predicting %>%
  mutate(finalOdds = ifelse(Season == "2017-18", finalsOdds2018(AdjustedNet), 0))
predicting <- predicting %>%
  mutate(confFinalOdds = ifelse(Season == "2017-18", confFinalsOdds2018(AdjustedNet), 0))
predicting <- predicting %>%
  mutate(secondRoundOdds = ifelse(Season == "2017-18", secondRoundOdds2018(AdjustedNet), 0))
