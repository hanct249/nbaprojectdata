...
champOdds2020 <- function(adjustedNet) {
  return(1/(1 + exp(-(-5.7081+(63.3313*adjustedNet)))))
}

predictionFinals <- glm(madeFinals ~ AdjustedNet, data = predicting, family = "binomial")
summary(predictionFinals)
finalsOdds2020 <- function(adjustedNet) {
  return(1/(1 + exp(-(-4.0738+(48.3723*adjustedNet)))))
}
predictionConfFinals <- glm(madeConfFinals ~ AdjustedNet, data = predicting, family = "binomial")
summary(predictionConfFinals)
confFinalsOdds2020 <- function(adjustedNet) {
  return(1/(1 + exp(-(-3.4137+(56.3304*adjustedNet)))))
}
predictionSecondRound <-glm(madeSecondRound ~ AdjustedNet, data = predicting, family = "binomial")
summary(predictionSecondRound)
secondRoundOdds2020 <- function(adjustedNet) {
  return(1/(1 + exp(-(-2.2165+(65.8215*adjustedNet)))))
}

predicting <- predicting %>%
  mutate(championOdds = ifelse(Season == "2019-20", champOdds2020(AdjustedNet), 0))
predicting <- predicting %>%
  mutate(finalOdds = ifelse(Season == "2019-20", finalsOdds2020(AdjustedNet), 0))
predicting <- predicting %>%
  mutate(confFinalOdds = ifelse(Season == "2019-20", confFinalsOdds2020(AdjustedNet), 0))
predicting <- predicting %>%
  mutate(secondRoundOdds = ifelse(Season == "2019-20", secondRoundOdds2020(AdjustedNet), 0))
