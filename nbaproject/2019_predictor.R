...
champOdds2019 <- function(adjustedNet) {
  return(1/(1 + exp(-(-5.348+(59.2544*adjustedNet)))))
}

predictionFinals <- glm(madeFinals ~ AdjustedNet, data = predicting, family = "binomial")
summary(predictionFinals)
finalsOdds2019 <- function(adjustedNet) {
  return(1/(1 + exp(-(-4.1383+(49.6523*adjustedNet)))))
}
predictionConfFinals <- glm(madeConfFinals ~ AdjustedNet, data = predicting, family = "binomial")
summary(predictionConfFinals)
confFinalsOdds2019 <- function(adjustedNet) {
  return(1/(1 + exp(-(-3.6182+(60.8598*adjustedNet)))))
}
predictionSecondRound <-glm(madeSecondRound ~ AdjustedNet, data = predicting, family = "binomial")
summary(predictionSecondRound)
secondRoundOdds2019 <- function(adjustedNet) {
  return(1/(1 + exp(-(-2.281+(66.725*adjustedNet)))))
}

predicting <- predicting %>%
  mutate(championOdds = ifelse(Season == "2018-19", champOdds2019(AdjustedNet), 0))
predicting <- predicting %>%
  mutate(finalOdds = ifelse(Season == "2018-19", finalsOdds2019(AdjustedNet), 0))
predicting <- predicting %>%
  mutate(confFinalOdds = ifelse(Season == "2018-19", confFinalsOdds2019(AdjustedNet), 0))
predicting <- predicting %>%
  mutate(secondRoundOdds = ifelse(Season == "2018-19", secondRoundOdds2019(AdjustedNet), 0))
